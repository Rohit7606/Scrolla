...
| 9 | 2026-09-06 | b/health-card-cleanup (`service/`, `room/`) | **M1** | **B (self-review — A unavailable)** | Approved with caveat | **Fixes for review #8.** Nine of eleven audit findings resolved: A1 reset guard now contributes zero and moves the baseline, with the arithmetic extracted to `service/ScrollDelta.kt` as a pure function and pinned by 15 new tests; A2 double-checked lock; A3 personal best excludes today and zero days; A4 `@Index("day")` + `MIGRATION_2_3` (schema v3); A6 `markDegraded` and its nine call sites removed so reads no longer write service health; A7 2-arg `startForeground` below API 30; A8 `getOnce()`, `upsert()`, `getTotalCmBetweenDays()` deleted; A9 baselines bounded by a 500-entry LRU. A5 (`AppTotal`) and A4's retention half left open as decisions for A. 79 tests pass. Migration verified by upgrading the **live device database** in place — v2→v3, 2044 events preserved, plan changed from `SCAN` to `SEARCH … USING INDEX` — rather than by reading the SQL, and A3 verified by running old and new queries against that same data (old: today at 6.75 m; new: 2026-09-04 at 21.55 m). **CAVEAT: every one of these changes is in A's layer (`service/`, `room/`) which §2 assigns to A, and none has been seen by a second person.** Review #8 stays "Changes required" until A re-checks — per this document's own process the reviewer, not the author, closes that out. Two things this review cannot cover: A7 is untested on any API 24–29 device, and `SENSOR_PROGRESS.md`'s accuracy figures were all measured with A1 present, so S0.7/S0.8 must be re-run. | No — A to retro-review |

| 10a | 2026-09-06 | b/health-card-cleanup (`firestore.rules`) | **M2** | B | **Changes required — self-caught** | **The first version of the rule below omitted `lastUpdated`, which would have silently broken every restore.** `RecordEligibility.isPlausiblyComplete()` opens `if (total.lastUpdated <= 0L) return false`, and `BackupRepository.fetchAll` defaulted the field to 0 on the reasoning that a device-local health signal should not be fabricated. That reasoning was fine and stopped one step too early: it never traced what consumes the field. The effect would have been a user reinstalling, appearing to get their whole history back, and finding that none of it could ever set a group record — with nothing to indicate why. Same shape as audit A1: the visible half worked. Fixed by carrying `lastUpdated` through the document, validating it in the rule (`is int && >= 0`), and making the reconcile re-upload any cloud row whose timestamp is missing so documents written before the fix repair themselves. 4 more tests (91 total). | N/A |
| 10 | 2026-09-06 | b/health-card-cleanup (`firestore.rules`, `firestore/`) | **M2** | **PENDING — A must review before deploy** | *Not yet reviewed* | **Personal daily-total backup (P2.16).** Adds `match /users/{userId}/dailyTotals/{day}`: owner-only read/delete, and create/update additionally constrained to a `YYYY-MM-DD` doc id, `hasOnly(['date','totalKm','updatedAt'])`, `date == day`, and `totalKm is number && >= 0`. Its own match block is required because rule 1's `allow write` on `/users/{userId}` does not cascade to subcollections — without it every write is denied. Deliberately **not** readable by other signed-in users, unlike `groups/{g}/dailyTotals`, since nobody needs another person's full history. Client side: `BackupRepository` (batched writes, chunked at 500), `BackupReconcile` (pure, 8 tests, device-wins), reconcile wired into `SyncViewModel` once per session before the group sync so a restored day exists before records are offered, and `deleteAllUserData` extended to delete the backup — an orphan here would be unreachable and undeletable, as every rule is gated on `request.auth.uid`. **§2 requires A on rules changes and this has not had it. DO NOT DEPLOY until A signs off.** Separately, this session confirmed by diffing against the Firebase console that the *existing* rules (reviews #5 and #6) have been live for some time, contradicting PREMIUM_CHECKLIST, which had claimed "not deployed" since 2026-08-25. | N/A — not deployed |
