...
| S0.3 | Raw `TYPE_VIEW_SCROLLED` events logging to Logcat with `packageName`, `scrollY`, and the per-view key (`packageName:className:viewId`). | ☑ | 2026-07-10 | AI — Verified per-view delta tracking and logcat output. |
| S0.4 | Per-view HashMap delta tracking implemented per `AGENTS.md` Section 4.2 and `DATA_CONTRACT.md` Section 2. Global `lastScrollY` does not exist anywhere in the codebase. | ☑ | 2026-07-10 | AI — Verified implementation in ScrollAccessibilityService. |
...
| S0.6 | `DistanceFormatter.pxToCm()` wired in. Accumulated distance logs in cm, not pixels. | ☐ | | |
| S0.7 | **PHYSICAL DEVICE ACCURACY TEST — required.** Scroll continuously in one app for 5 minutes on a real phone (not emulator). Log the total cm reported. Estimate manually (roughly 10–20 cm per swipe × ~30 swipes per minute × 5 min = 1,500–3,000 cm). Confirm reported value is within 2× of the estimate. Log result in `DEVICE_TEST_LOG.md`. A 10× discrepancy means the delta logic is wrong — do not proceed until this passes. | ☐ | | |
| S0.8 | Accuracy test repeated on a second physical device (preferably a different manufacturer from S0.7). Log in `DEVICE_TEST_LOG.md`. | ☐ | | |
| S0.9 | App switch test: scroll in Instagram, switch to Reddit, scroll again. Confirm zero phantom distance from the app switch (i.e. the delta between Instagram's last position and Reddit's first position is not accumulated). | ☐ | | |
